...
export function registerDeliveryLineActionTools(server: McpServer, scope: McpScope) {
  ...
  server.tool(
    ...
    async ({ venueId, orderId, limit, cursor, blockedCursor }) => {
      ...
      return text(await listDeliveryLineActions(venueId, { orderId, limit, cursor, blockedCursor }))
    },
  )
}
