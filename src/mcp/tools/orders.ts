...
export function registerOrderTools(server: McpServer, scope: McpScope) {
  ...
  server.tool(
    'pos_sync_status',
    'Offline-first: últimos intents que los POS del venue reprodujeron al reconectar (replay del outbox offline) — 14 tipos: abrir/cobrar (OPEN_TABLE/ADD_ITEMS/PAY_CASH), mutaciones de cuenta (APPLY_DISCOUNT/APPLY_SERVICE_CHARGE/COMP_ORDER/UPDATE_DETAILS/CANCEL_ORDER/MOVE_ORDER/ASSIGN_ORDER/CLEAR_TABLE), y separar/fusionar cheques (SPLIT_ORDER/SPLIT_BY_SEAT/MERGE_ORDERS), dispositivo, ACKED / REJECTED con su errorCode / RETRY (transitorio, se reintenta solo). Answers "¿qué se sincronizó cuando volvió el internet? ¿algún cobro/comanda offline fue rechazado?". Pass venueId.',
    {
