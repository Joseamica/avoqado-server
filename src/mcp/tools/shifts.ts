...
export function registerShiftTools(server: McpServer, scope: McpScope) {
  ...
  server.tool(
    ...
    'History of PHYSICAL cash drawer sessions (arqueos de caja) for a venue you can access, newest first, open and closed alike. Each session: who opened/closed it and when, device, starting cash, cash sales, pay-ins, pay-outs, the expected cash (open drawers: only with `cash-drawer:view-expected`), whether the cashier actually COUNTED at close (`counted`), the counted amount and the over/short (positive = sobrante, negative = faltante). A session with counted=false was closed WITHOUT counting — never treat it as reconciled. `shiftId` links the drawer to its shift (turno de caja) when both were opened by the same gesture; null for older sessions. Answers "¿hubo faltante ayer? ¿quién cerró la caja el martes? ¿cuántas veces cerraron sin contar?". Pass venueId. Read-only.',
    {
