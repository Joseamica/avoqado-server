...
export function registerActivityLogTools(server: McpServer, scope: McpScope) {
  ...
  server.tool(
    'get_activity_log',
    'Audit trail for your venue(s): who did what and when — orders (comp/void/discount), payments, refunds, shifts, staff/access changes, inventory receiving and SIM custody, config changes, and manager-PIN authorizations. PAYMENT_WITHOUT_SHIFT means real money was preserved without a cash shift and data.reason distinguishes NO_SHIFT, SHIFT_NOT_OPEN, CLAIM_LOST, ORDER_AUTHORITY_UNAVAILABLE, or an unclassified historical case; it requires reconciliation. ORDER_AUTHORITY_UNAVAILABLE means the processor may have captured money but the tenant-scoped Order lock was lost, so the requester must reconcile manually without writing into the Order’s current tenant. PERMISSION_OVERRIDE_USED = a manager authorized a blocked action for someone else, one action and one time; the approver is in data.authorizedByStaffVenueId. PERMISSION_OVERRIDE_INSUFFICIENT = someone typed a valid PIN that does NOT carry that permission — the classic internal-fraud signal. PERMISSION_OVERRIDE_REJECTED = an expired or already-used token. PERMISSION_DENIED = a plain denial, no PIN attempted — only WRITE attempts (POST/PUT/PATCH/DELETE) and cross-venue access attempts are recorded; a denied READ is the app asking for a screen the role cannot see and is deliberately not audited, since it flooded the trail and buried real events. Most recent first. Pass venueId to focus one venue; filter by action code or date range. Requires the activity:read permission (owner-level).',
    {
      ...
      action: z
        ...
        .optional()
        .describe(
          'Filter by exact action code, e.g. PAYMENT_COMPLETED, PAYMENT_WITHOUT_SHIFT, SIM_CUSTODY_ASSIGNED_TO_PROMOTER or PERMISSION_OVERRIDE_USED (manager-PIN approvals; the approver is in data.authorizedByStaffVenueId)',
        ),
