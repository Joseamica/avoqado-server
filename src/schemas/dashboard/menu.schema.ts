...
const ProductBodyBase = z.object({
  ...
  // Tiempo de limpieza/preparación DESPUÉS del servicio. Aparta agenda pero NO
  // mueve la hora de fin que ve el cliente. Misma advertencia que `duration`:
  // sin esta línea Zod haría strip del campo y el dashboard guardaría en vacío.
  bufferAfterMin: z.number().int().min(0).max(240).optional().nullable(),
