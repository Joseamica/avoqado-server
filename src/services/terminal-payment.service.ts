...
export interface TerminalPaymentResult {
  ...
  /**
   * Lo que la TERMINAL contestó antes de que el servidor degradara el resultado a `timeout` por venir sin evidencia (o su
   * propio `timeout`: «no pude verificar»). Se conserva sólo para diagnóstico y bitácora (qué código del SDK fue); NUNCA
   * acredita nada. Su presencia es lo que distingue TIMED_OUT (la terminal contestó: entra en la ventana de confirmación)
   * de UNKNOWN (nadie afirmó nada útil).
   */
  terminalResult?: { status: 'failed' | 'cancelled' | 'timeout'; errorMessage: string | null; outcomeEvidence: string | null }
  /**
   * Codex r1 (P1-D): lo que la TERMINAL AFIRMÓ en un `success` que el servidor no pudo acreditar (Payment inexistente o no
   * elegible, o error verificando). Se conserva tal cual —sólo las llaves presentes— al degradar a `timeout` SIN `terminalResult`
   * (la fila queda UNKNOWN, fuera de la ventana): la declaración del cajero lo lee como señal positiva y NO cierra encima. Un
   * negativo posterior de la terminal (camino `late`) reemplaza el sobre entero y lo pierde — coherente con el ruling (c) de la
   * Task 2: la terminal que contradice su propio `success` con un negativo sin evidencia entra en la ventana, y ahí el veto
...
/**
 * Ronda 3 (P1-A): el horizonte de la RED DURABLE — «¿hay un Payment LIGADO a una solicitud liberada?» ya no depende de los 30
 * minutos de arriba. Se mide por `createdAt` (nunca por `updatedAt`, que la propia re-retención mueve) y son 7 días: acota el
 * recorrido y cubre con holgura el peor registro tardío medido (65 s – 3 h). Más antiguo que eso es conciliación manual.
 */
const HORIZONTE_DE_LA_RED_DURABLE_MS = 7 * 24 * 60 * 60_000
 ...
 * exige `terminalResult` para NO alcanzar filas TIMED_OUT históricas sin sobre (anteriores a este cambio). Sin esto, en los
 * venues con el interruptor apagado (el heredado sólo retenía `SLOT_HELD`) una fila de la ventana soltaría la ranura
 * AL INSTANTE, antes de que la ventana decidiera (Codex, Task 0 R2, P1). La excepción de `AUTO_RELEASED` no se toca.
 * Revisión final (17-sep, D): «fila de la ventana» es la MISMA en las cuatro lecturas (este predicado, su espejo
 * `ventanaRetieneLaRanura`, el barrido y `esNegativoSinEvidencia`): sobre con `status: 'timeout'` Y `terminalResult.status` cadena.
 * Una TIMED_OUT sin código con `terminalResult` pero sin ese `status` (ausente o distinto) la ventana no la liberaría nunca, así
 * que tampoco la retiene por ella. La comparación es NULL-segura: con la llave ausente la rama no aplica (no hay `NOT` alrededor).
 */
    ...
    { OR: [{ paymentId: null }, { paymentId: '' }] },
    {
      OR: [
        { failureCode: { in: [...MARCADORES_DE_RETENCION_DE_LA_VENTANA] } },
        {
          failureCode: null,
          AND: [
            { resultJson: { path: ['terminalResult', 'status'], string_contains: '' } },
            { resultJson: { path: ['status'], equals: 'timeout' } },
          ],
        },
      ],
    },
 ...
 * `status: { in: SLOT_HELD }`) más las filas de la ventana de confirmación (`VENTANA_RETIENE_LA_RANURA`). No se deriva
 * de la lista blanca a propósito — es el comportamiento heredado, y tiene que poder leerse y compararse contra el de
 * producción sin desenredar nada.
 */
...
export function bloqueaLaRanura(row: FilaDeBloqueo, estrictos: EstrictoPorVenue): boolean {
  ...
  if (ventanaRetieneLaRanura(row)) return true
...
export function ventanaRetieneLaRanura(row: FilaDeDesenlace): boolean {
  ...
  const sobre =
    row.resultJson && typeof row.resultJson === 'object' && !Array.isArray(row.resultJson)
      ...
      : null
  // Revisión final (17-sep, D): el sobre tiene que decir `timeout` — la misma definición que el SQL de arriba y que la ventana.
  if (sobre?.status !== 'timeout') return false
...
/** El instante en que el CAJERO declaró «no se presentó tarjeta», tal como lo dejó en el sobre; `null` si no está o no se lee. */
function instanteDeDeclaracion(resultJson: Prisma.JsonValue | null): Date | null {
  const sobre = resultJson && typeof resultJson === 'object' && !Array.isArray(resultJson) ? (resultJson as Record<string, unknown>) : null
  const declaracion = sobre?.operatorResolution
  const acceptedAt = declaracion && typeof declaracion === 'object' ? (declaracion as Record<string, unknown>).acceptedAt : undefined
  if (typeof acceptedAt !== 'string') return null
  const fecha = new Date(acceptedAt)
  return Number.isNaN(fecha.getTime()) ? null : fecha
}

/**
 * Revisión final (17-sep, B): los dos códigos con los que el SERVIDOR soltó una venta SIN evidencia del procesador — la ventana
 * (`NO_EVIDENCE_AFTER_WINDOW`) y el testimonio del cajero (`OPERATOR_RECONCILED_NO_CHARGE`). Son los que esta regla re-retiene
...
function liberadaSinCobroReversible(row: { status: TerminalPaymentRequestStatus; failureCode: string | null; paymentId: string | null }) {
  return (
    ...
    CODIGOS_DE_LIBERACION_REVERSIBLE.includes(row.failureCode ?? '') &&
    row.paymentId == null
  )
}

/** Reconstruct a client-facing result from a stored row (for idempotent replay). */
/** Codex r1 (P1-D): las llaves con las que una terminal AFIRMA haber cobrado en un `success`. Sólo las presentes; nada se inventa. */
const LLAVES_DE_LA_AFIRMACION = ['paymentId', 'authorizationCode', 'transactionId', 'reference', 'readMode', 'approved'] as const
function claimedSuccessDe(result: TerminalPaymentResult): Record<string, unknown> {
  const crudo = result as unknown as Record<string, unknown>
  const afirmacion: Record<string, unknown> = {}
  for (const llave of LLAVES_DE_LA_AFIRMACION) if (crudo[llave] !== undefined) afirmacion[llave] = crudo[llave]
  return afirmacion
}
 ...
 */
/**
 * Ronda 3 (17-sep, P1-B): `IDENTITY_MISMATCH` = la señal existe pero la identidad ACREDITADA de quien la trae no es la
 * terminal de esa solicitud (regla T10). No se tocó la fila y NO hay nada que reintentar — a diferencia de `DEFERRED`.
 */
export type ResultadoDeReRetencion = 'HELD' | 'NOT_APPLICABLE' | 'DEFERRED' | 'IDENTITY_MISMATCH'
...
export type OrigenDeRetencionPorPago =
  ...
  | 'BARRIDO_LIBERADAS'
  | 'LIBERACION_MANUAL'
  /** Ronda 3 (P1-A): el backfill del webhook, ANTES de sellar el evento con la llave del Payment. */
  | 'BACKFILL'
 ...
 *  · `PAGO_SIN_LIGAR`           hay un Payment ligado que nadie pudo LIGAR                  ⇒ `PAYMENT_UNBOUND_AWAITING_REVIEW`
 *  · `COLISION_DE_REFERENCIA`   hay EVIDENCIA PENDING de conciliación apuntada a la fila    ⇒ `PAYMENT_UNBOUND_AWAITING_REVIEW`
 *  · `AFIRMACION_DE_LA_TERMINAL` la propia terminal AFIRMÓ haber cobrado, sin Payment       ⇒ `PAYMENT_UNBOUND_AWAITING_REVIEW`
 */
type VarianteDeReRetencion =
  ...
  | { tipo: 'COLISION_DE_REFERENCIA'; paymentId: string; capturedBySerial: string | null; origen: OrigenDeRetencionPorPago }
  | { tipo: 'AFIRMACION_DE_LA_TERMINAL'; origen: OrigenDeRetencionPorPago }

/**
 * Lo único que cambia entre las cuatro variantes del núcleo: la llave del sobre, la acción del asiento, el texto que lee el
 * POS, y los textos del 🚨 y del correo. Las dos primeras conservan su contrato EXACTO (la ronda 1 y la 2 tienen pruebas que
 * fijan el asunto y la forma de `data`). `instante` es el nombre del campo con la hora de la retención: `heldAt` es el
 * histórico de esas dos; las nuevas usan `at`, el del brief.
 */
const MARCA_DE_LA_RETENCION: Record<
  VarianteDeReRetencion['tipo'],
  { claveDelSobre: string; accion: string; mensaje: string; asunto: string; grito: string; instante: 'heldAt' | 'at' }
> = {
  APROBACION: {
    claveDelSobre: 'bankApprovedAfterRelease',
    accion: 'TERMINAL_PAYMENT_WINDOW_BANK_APPROVED_AWAITING_PAYMENT',
    mensaje: 'El banco aprobó este cobro después de liberarlo y todavía no está registrado. No lo cobres otra vez: se está revisando.',
    asunto: 'Aprobación del banco sobre un cobro ya liberado',
    grito: '',
    instante: 'heldAt',
  },
  PAGO_SIN_LIGAR: {
    claveDelSobre: 'unboundPaymentAfterRelease',
    accion: 'TERMINAL_PAYMENT_UNBOUND_PAYMENT_AFTER_RELEASE',
    mensaje:
    ...
    asunto: 'Cobro con tarjeta sin ligar sobre un cobro ya liberado',
    grito: '',
    instante: 'heldAt',
  },
  COLISION_DE_REFERENCIA: {
    claveDelSobre: 'referenceCollisionAfterRelease',
    accion: 'TERMINAL_PAYMENT_REFERENCE_COLLISION_AFTER_RELEASE',
    mensaje: 'Llegó un registro con la referencia de otro cobro después de liberar este intento. No lo cobres otra vez: se está revisando.',
    asunto: 'Colisión de referencia sobre un cobro ya liberado',
    grito: 'A reference-collision evidence landed on a request already RELEASED (window/cashier) — request held for review',
    instante: 'at',
  },
  AFIRMACION_DE_LA_TERMINAL: {
    claveDelSobre: 'terminalClaimedSuccessAfterRelease',
    accion: 'TERMINAL_PAYMENT_TERMINAL_CLAIM_AFTER_RELEASE',
    mensaje: 'La terminal afirmó haber cobrado este intento después de liberarlo. No lo cobres otra vez: se está revisando.',
    asunto: 'La terminal afirmó haber cobrado un cobro ya liberado',
    grito: 'The terminal CLAIMED it charged a request already RELEASED (window/cashier) and no Payment proves it — held for review',
    instante: 'at',
  },
}
...
class TerminalPaymentService {
  ...
  /** requestId → temporizador en proceso de la VENTANA DE CONFIRMACIÓN (la rapidez; el watchdog es la durabilidad). */
  ...
  async handlePaymentResultFromSocket(
    ...
    // no hay desenlace. Contestarle al POS en este instante era un 504 inmediato: sondeaba, leía TIMED_OUT sin veredicto y se
    // quedaba en «confirmando» hasta un «Volver a consultar» manual. La espera sigue viva y la contesta QUIEN DECIDE: la ventana
    // (`releaseUnprovenNegative` → `resolverEsperaDelPos`), la declaración del cajero, el dinero (`closeRow` de un `success`, S5
    // `confirmFromWebhook`, `resolvePendingFromDurableState`) o, si nadie decide, su propio plazo de siempre. Un `timeout` SIN
    // sobre (UNKNOWN: success inacreditable, error verificando) se contesta como siempre.
    if (outcome.status === 'timeout' && outcome.terminalResult) {
      logger.info('⏳ [TerminalPayment] Result entered the confirmation window — the POS keeps waiting for the decision', {
        requestId: result.requestId,
        venueId: terminal.venueId,
      })
      return true
  ...
  /**
   * Revisión final (17-sep, A): entrega a la espera del POS (long-poll en memoria) el desenlace DURABLE de la fila — sólo si ya
   * hay uno (`desenlaceCanonico` ≠ UNRESOLVED: la liberación por ventana, la declaración del cajero, el dinero). Una fila sin
   * desenlace (en la ventana, retenida por el banco o por un Payment sin ligar) no se contesta: el POS sigue esperando. La
   * espera de OTRO venue no se toca, y sin espera en memoria no se consulta la base. Devuelve si despertó a alguien.
   */
  async resolverEsperaDelPos(requestId: string, venueId: string): Promise<boolean> {
    const pending = this.pendingPayments.get(requestId)
    if (!pending || pending.venueId !== venueId) return false
    const row = await prisma.terminalPaymentRequest.findFirst({
    ...
    if (!row || desenlaceCanonico(row).outcome === 'UNRESOLVED') return false
    // La lectura es asíncrona: otro camino pudo contestar (y borrar) la espera mientras tanto. Sólo se resuelve la MISMA.
    if (this.pendingPayments.get(requestId) !== pending) return false
    clearTimeout(pending.timeout)
    this.pendingPayments.delete(requestId)
    pending.resolve(resultFromRow(row))
    logger.info('🔔 [TerminalPayment] POS long-poll answered with the durable decision', {
      requestId,
      venueId,
      status: row.status,
  ...
  private async closeRow(requestId: string, venueId: string, result: TerminalPaymentResult): Promise<TerminalPaymentResult> {
    ...
    // Revisión final (17-sep, C): la llave se descarta SIEMPRE que venga, pero el 🚨 sólo con un id real (`!= null`): la TPV
    // publicada (2.9.2) serializa `"paymentId": null` en TODO failed/cancelled, y eso no es una anomalía sino su forma de decir
    // «sin pago» — con `!== undefined` eran varias alertas falsas al día por terminal que tapaban las verdaderas.
    ...
    if (result.claimedSuccess) {
      const escrito = await this.escribirSuccessDegradado(requestId, venueId, result, newStatus)
      // 🔴 Ronda 3 (P1-C): la afirmación ya está DURABLE en el sobre (la escribió la primera sentencia de arriba, que alcanza
      // también a una fila ya liberada). Si la fila estaba LIBERADA, esa afirmación positiva la RE-RETIENE para revisión — con
  ...
  private esNegativoSinEvidencia(row: {
    status: TerminalPaymentRequestStatus
    failureCode: string | null
    paymentId: string | null
    resultJson: Prisma.JsonValue | null
    ...
    const sobre =
      row.resultJson && typeof row.resultJson === 'object' && !Array.isArray(row.resultJson)
        ? (row.resultJson as Record<string, unknown>)
        : null
    // «Fila de la ventana» se define igual en las cuatro lecturas (espejo `ventanaRetieneLaRanura`, predicado
    // `VENTANA_RETIENE_LA_RANURA`, barrido y aquí): el sobre dice `status: 'timeout'` y `terminalResult.status` es una cadena.
  ...
  /**
   * La VENTANA DE CONFIRMACIÓN decide sobre UNA solicitud: un negativo de la terminal sin evidencia que lleve ≥ 30 s se
   * libera (`FAILED/NO_EVIDENCE_AFTER_WINDOW`), salvo que el pago EXACTO ya exista (se concilia) o que un webhook APROBADO
   * de un intento vinculado ya conste sin Payment (se RETIENE con `BANK_APPROVED_AWAITING_PAYMENT`, que sigue UNRESOLVED).
   * Idempotente por CAS: la llaman el temporizador en proceso (`TIMER`) y el barrido del watchdog (`WATCHDOG`).
   */
  async releaseUnprovenNegative(
    ...
    if (payment) {
      ...
      if (cierre.bound) {
        logger.warn('🕰️ [TerminalPayment] window found the exact payment — reconciled instead of released', {
          requestId,
          venueId,
          paymentId: payment.id,
        })
        ...
        // Revisión final (A): la espera del POS sigue viva desde que la fila entró a la ventana — se le contesta el cobro ya.
        await this.resolverEsperaDelPos(requestId, venueId).catch(err =>
          logger.warn('⚠️ [TerminalPayment] Could not answer the POS after the window reconciliation', {
            requestId,
            venueId,
            error: err instanceof Error ? err.message : String(err),
          }),
    ...
    // 2+3) UNA fotografía bajo dos candados, revalidada en la ESCRITURA (Codex r1, P1-C):
    //  · candado de la SOLICITUD primero (orden fijo solicitud → intentos; lo toman también la publicación del vínculo y el
    //    fallback del webhook), y los vínculos se enumeran DENTRO, después de él: un vínculo que se publique mientras decidimos
    //    espera, y uno publicado antes se ve;
    //  · candado de CADA intento vinculado (el ingreso del webhook toma el mismo): un APROBADO que se persista después espera;
    //    uno que se persistió antes lo vemos (insertar el evento no toca el updatedAt de la solicitud, así que el CAS solo NO
    //    bastaba — Codex, Task 0 R2, P3);
    //  · el CAS de liberación lleva además `NOT EXISTS` de APROBADO vinculado y de Payment ligado (`sinEvidenciaPositivaSql`)
    //    sobre lo durable en ese instante: un escritor SIN candado (fallback vencido por lock_timeout, cola vieja etiquetando
    //    sólo `processorData`) que entre entre el veto y la escritura no libera. Un `$executeRaw` que devuelve 0 es NOT_ELIGIBLE.
    const previo =
      row.resultJson && typeof row.resultJson === 'object' && !Array.isArray(row.resultJson) ? (row.resultJson as Prisma.JsonObject) : {}
    ...
    try {
      decision = await prisma.$transaction(async tx => {
        ...
        if (aprobacion) {
          ...
          // (`updatedAt` explícito: un UPDATE crudo no pasa por `@updatedAt`, y el destrabe de la ranura a los 20 min lo lee.)
          const held = await tx.$executeRaw`
            UPDATE "TerminalPaymentRequest"
            SET "failureCode" = 'BANK_APPROVED_AWAITING_PAYMENT', "updatedAt" = (NOW() AT TIME ZONE 'UTC')
            WHERE "id" = ${row.id} AND "status" = 'TIMED_OUT' AND "failureCode" IS NULL AND "paymentId" IS NULL
              AND "updatedAt" = ${utcTs(row.updatedAt)}`
          if (held === 0) return 'NOT_ELIGIBLE'
          await tx.activityLog.create({
            data: {
              action: 'TERMINAL_PAYMENT_WINDOW_HELD_BY_BANK_EVIDENCE',
              entity: 'TerminalPaymentRequest',
              entityId: row.id,
              venueId,
              data: {
        ...
        if (r === 0) {
          ...
          logger.warn('⏱️ [TerminalPayment] window release CAS lost — the request stays in the window', {
            requestId,
            venueId,
            origen,
            motivoProbable: motivo === 'NINGUNA' ? 'LA_FILA_CAMBIO' : motivo,
          })
          return 'NOT_ELIGIBLE'
        }
        await tx.activityLog.create({
          data: {
            action: 'TERMINAL_PAYMENT_RELEASED_AFTER_WINDOW',
            entity: 'TerminalPaymentRequest',
            entityId: row.id,
            venueId,
            data: {
              ...
              amountCents: row.amountCents,
              tipCents: row.tipCents,
              windowMs: UNPROVEN_NEGATIVE_WINDOW_MS,
              origen,
              terminalResult: ((previo as Record<string, unknown>).terminalResult ?? null) as Prisma.InputJsonValue | null,
            },
          },
        })
        return 'RELEASED'
      ...
      // 55P03 (lock_timeout): el ingreso del webhook tiene el candado de este intento, o la publicación de un vínculo / el
      // fallback tienen el de la solicitud — esta pasada no decide; la siguiente (temporizador o watchdog a los 30 s) vuelve a
      // intentar y verá lo ya persistido.
      logger.warn('⏱️ [TerminalPayment] window release deferred — attempt lock busy or db error', {
        requestId,
        venueId,
        error: err instanceof Error ? err.message : String(err),
      })
    ...
    // Un POS que todavía espere en memoria recibe el desenlace ahora, no cuando venza su propio plazo (revisión final, A: su
    // espera sigue viva desde que la fila entró a la ventana). Por el MISMO camino que la declaración: la fila durable manda, y
    // un fallo al despertarlo no deshace la liberación ya confirmada (el POS la lee por el GET).
    await this.resolverEsperaDelPos(requestId, venueId).catch(err =>
      logger.warn('⚠️ [TerminalPayment] Could not answer the POS after the window release', {
        requestId,
        venueId,
        error: err instanceof Error ? err.message : String(err),
      }),
  ...
  /**
   * Revisión final (17-sep, B — dinero): el banco APROBÓ un intento vinculado DESPUÉS de que la solicitud se liberara — por la
   * ventana (`FAILED/NO_EVIDENCE_AFTER_WINDOW`) o por el cajero (`FAILED/OPERATOR_RECONCILED_NO_CHARGE`) — y ese aviso NO creó el
   * Payment (importe distinto, el registrador cayó, el serial contradice el vínculo, colisión de referencia). Antes la fila seguía
   * FAILED y el POS decía «puedes volver a cobrar» con una aprobación del banco encima. Ahora vuelve a `TIMED_OUT` con el MISMO
   * marcador con el que la ventana la habría retenido un segundo antes (`BANK_APPROVED_AWAITING_PAYMENT`): orden y ranura
   * retenidas en los dos regímenes, UN asiento, 🚨 y correo; la ranura se suelta a los 20 min como siempre y el Payment, si
   * llega, la cierra por el cierre común.
   *
   ...
   *  · `DEFERRED` — no se pudo decidir (55P03 del candado, base caída): nada se escribió; quien llama deja su evento PENDING para
   *    que el siguiente intento del worker lo repita. Nunca lanza.
   */
  async retenerSolicitudLiberadaPorAprobacion(input: {
    requestId: string
    venueId: string
    attemptId: string
    eventLogId: string
  ...
  /**
   * Ronda 3 (17-sep, P1-B — dinero, preexistente: Codex r8 respuesta 6 concern 2): un registro legacy que SÍ nombra la
   * solicitud pero sin llave de intento, con referencia ambigua y CONTRADICCIÓN acreditada, produce EVIDENCIA PENDING
   * (`POSSIBLE_REFERENCE_COLLISION`), responde éxito al cajero… y dejaba la solicitud liberada INTACTA, diciéndole al POS
   * «puedes volver a cobrar» con una posible segunda captura encima. El barrido no la veía: sólo busca Payments COMPLETED.
   *
   * Ahora esa evidencia RE-RETIENE la fila para REVISIÓN con el mismo marcador de la familia
   * (`PAYMENT_UNBOUND_AWAITING_REVIEW`) y razón propia `REFERENCE_COLLISION_AFTER_RELEASE`. Tres reglas, todas del brief:
   *
   *  (a) La señal viene del REGISTRADOR, no del cuerpo del cliente: `paymentId` es la evidencia que el registrador acaba de
   *      crear, y el CAS exige que esa evidencia CONSTE en la base apuntada a la solicitud (`hayEvidenciaDeConciliacionSql`)
   *      — no puede apoyarse en `hayPagoLigadoSql`, porque una evidencia PENDING no liga por ninguna de las tres identidades.
   *  (b) 🔴 Sólo re-retiene si la identidad ACREDITADA del llamador es la terminal de esa solicitud (misma precedencia T10
   *      del cierre por REST: `Payment.terminal` → serial autenticado → `processorData.deviceSerialNumber`). La etiqueta de
   *      la evidencia nace del cuerpo del cliente, así que sin esta comprobación una terminal podría retener la venta y la
   *      ranura de OTRA. Si no corresponde: la fila NO se toca, 🚨 con la contradicción y `IDENTITY_MISMATCH`.
   *  (c) UN asiento (`TERMINAL_PAYMENT_REFERENCE_COLLISION_AFTER_RELEASE`), 🚨 y correo, como el resto de las variantes.
   */
  async retenerSolicitudLiberadaPorColisionDeReferencia(input: {
    requestId: string
    venueId: string
    paymentId: string
    capturedBySerial?: string | null
    ...
    return this.reRetenerSolicitudLiberada(input.requestId, input.venueId, {
      tipo: 'COLISION_DE_REFERENCIA',
      paymentId: input.paymentId,
      capturedBySerial: input.capturedBySerial ?? null,
      origen: input.origen,
    })
  }

  /**
   * Ronda 3 (17-sep, P1-C — dinero, preexistente: Codex r8 respuesta 6 concern 4): un `success` TARDÍO de la propia terminal
   * con `transactionId` pero SIN Payment acreditable funde su `claimedSuccess` sobre la fila FAILED ya liberada… y la fila
   * seguía proyectando NOT_CHARGED con la ranura YA LIBRE. Es exactamente el caso que la ventana RETIENE un segundo antes de
   * liberar (Codex r5 P1-N5: `retenerPorPagoSinLigar` con razón `TERMINAL_CLAIMED_SUCCESS`); esto lo hace DESPUÉS, con el
   * MISMO marcador y la MISMA razón — y al volver a `TIMED_OUT` **recupera la ranura** además de marcar la fila.
   *
   * El CAS es el POSITIVO del veto de la ventana (`NOT (SIN_AFIRMACION_DE_LA_TERMINAL_SQL)`): la afirmación tiene que constar
   * DURABLE en el sobre — la persiste `escribirSuccessDegradado` en su primera sentencia, antes de pelear por el estado. Lo
   * que la ventana no habría vetado (una afirmación vacía: ninguna llave útil) tampoco se retiene aquí.
   *
   * La identidad ya la comprobó quien entrega el resultado (`handlePaymentResultFromSocket` y `handleProbeResultFromSocket`
   * exigen que la solicitud sea de la terminal autenticada antes de llamar a `closeRow`): una afirmación de OTRA terminal no
   * llega hasta aquí, y tampoco puede escribir el `claimedSuccess` que este CAS exige.
   */
  async retenerSolicitudLiberadaPorAfirmacionDeLaTerminal(input: {
    requestId: string
    venueId: string
    origen: OrigenDeRetencionPorPago
  }): Promise<ResultadoDeReRetencion> {
    return this.reRetenerSolicitudLiberada(input.requestId, input.venueId, { tipo: 'AFIRMACION_DE_LA_TERMINAL', origen: input.origen })
  }
  ...
  private async reRetenerSolicitudLiberada(
    ...
    const deLaVariante: Record<string, string | null> =
      variante.tipo === 'APROBACION'
        ...
        : variante.tipo === 'COLISION_DE_REFERENCIA'
          ? { paymentId: variante.paymentId, reason: 'REFERENCE_COLLISION_AFTER_RELEASE', origen: variante.origen }
          : variante.tipo === 'AFIRMACION_DE_LA_TERMINAL'
            ? { reason: 'TERMINAL_CLAIMED_SUCCESS', origen: variante.origen }
            : { reportedPaymentId: variante.paymentId, origen: variante.origen }
    // La marca de cada variante: el código que se escribe, la llave del sobre, la acción del asiento y los textos. Los dos de
    // la ronda 1 y 2 conservan su contrato EXACTO (asunto, primera línea y forma de `data`).
    const marca = MARCA_DE_LA_RETENCION[variante.tipo]
    // Ronda 3 (P1-B): la contradicción de identidad se decide BAJO los candados y se grita DESPUÉS, sin escribir nada.
    ...
    try {
      ...
      // Ronda 3 (P1-C): sin afirmación en el sobre no hay nada que retener por esta variante — y no se abre transacción. Es el
      // espejo del veto de la ventana (`afirmacionDeLaTerminal` / `SIN_AFIRMACION_DE_LA_TERMINAL_SQL`), que el CAS revalida.
      if (variante.tipo === 'AFIRMACION_DE_LA_TERMINAL' && !afirmacionDeLaTerminal(vista.resultJson)) return 'NOT_APPLICABLE'
      ...
      type Retenida = {
        ...
        ligados: string[]
        afirmacion: Record<string, unknown> | null
      }
      const retenida = await prisma.$transaction(async (tx): Promise<Retenida | null> => {
        ...
        const attemptIds = (await tx.terminalPaymentAttemptLink.findMany({ where: { requestId, venueId }, select: { attemptId: true } }))
          ...
          .sort()
        for (const attemptId of attemptIds) await candadoDeIntento(tx, attemptId)
        // Relectura BAJO los candados: la ventana, el cajero o el cierre común pudieron moverla entre la vista y aquí.
        const row = await tx.terminalPaymentRequest.findFirst({ where: { requestId, venueId } })
        ...
        const ligados = variante.tipo === 'PAGO_SIN_LIGAR' ? (await pagosLigados(tx, requestId, venueId)).map(p => p.id) : []
        // La afirmación de la terminal, leída BAJO los candados: es lo que el CAS de su variante revalida en la escritura.
        const afirmacion = variante.tipo === 'AFIRMACION_DE_LA_TERMINAL' ? afirmacionDeLaTerminal(row.resultJson) : null
        // 🔴 Ronda 3 (P1-B), regla T10: la EVIDENCIA la lee el servidor —nunca se le cree al cuerpo del cliente— y su
        // procedencia se juzga con la MISMA precedencia y la MISMA función que el cierre por REST. Sin evidencia en la base no
        // hay señal (NOT_APPLICABLE); con identidad que no corresponde, la fila NO se toca.
        if (variante.tipo === 'COLISION_DE_REFERENCIA') {
          const evidencia = await tx.payment.findFirst({
            where: { id: variante.paymentId, venueId },
            select: { processorData: true, terminalPaymentRequestId: true, terminal: { select: { serialNumber: true } } },
          })
          if (!evidencia) return null
          const procedencia = procedenciaDelPagoDeSolicitud(
            evidencia as PagoConProcedencia,
            { requestId, orderId: row.orderId, terminalId: row.terminalId },
            { fase: 'ligar', capturedBySerial: variante.capturedBySerial },
          )
          if (!procedencia.acreditada) {
            contradiccion = procedencia.reason
            return null
          }
        }
        ...
        const detalle = {
          ...(variante.tipo === 'PAGO_SIN_LIGAR' ? { paymentIds: ligados } : {}),
          ...(afirmacion ? { claimedSuccess: afirmacion } : {}),
          ...deLaVariante,
          previousFailureCode,
          releasedAt: liberadaEn.toISOString(),
          ...
          // `heldAt` es el nombre histórico de las dos primeras variantes; las nuevas lo llaman `at` (el del brief).
          ...(marca.instante === 'at' ? { at: ahora } : { heldAt: ahora }),
        }
        ...
        const sobreRetenido = {
          ...
          errorMessage: marca.mensaje,
          [marca.claveDelSobre]: detalle,
        }
        // La evidencia de la variante NO-aprobación, evaluada EN la escritura: exactamente lo que el veto de la ventana habría
        // visto un instante antes de liberar. Se arma AQUÍ, bajo los candados, junto al CAS que la usa.
        const evidenciaDeLaRetencion =
          variante.tipo === 'PAGO_SIN_LIGAR'
            ? hayPagoLigadoSql(requestId, venueId)
            : variante.tipo === 'COLISION_DE_REFERENCIA'
              ? hayEvidenciaDeConciliacionSql(requestId, venueId)
              : Prisma.sql`NOT (${SIN_AFIRMACION_DE_LA_TERMINAL_SQL})`
        ...
        await tx.activityLog.create({
          data: {
            ...
            data: {
              requestId,
              ...(variante.tipo === 'PAGO_SIN_LIGAR' ? { paymentIds: ligados } : {}),
              ...(afirmacion ? { claimedSuccess: afirmacion as Prisma.InputJsonValue } : {}),
              ...deLaVariante,
              previousFailureCode,
              otherCardPaymentsOnOrderAfterRelease: otros,
              otherUnresolvedRequestsOnOrderAfterRelease: solicitudesSinDesenlace,
        ...
        return { row, previousFailureCode, otros, solicitudesSinDesenlace, ligados, afirmacion }
      }, OPCIONES_DE_TRANSACCION_DEL_INTENTO)
      if (!retenida) {
        // Ronda 3 (P1-B): la señal existe pero la trae quien no es la terminal de la solicitud. No se escribe nada y se grita:
        // es una contradicción que alguien tiene que mirar, no un reintento.
        if (contradiccion) {
          logger.error(
            '🚨 [TerminalPayment] A reconciliation evidence claims a RELEASED request from a terminal that does not own it — row untouched',
            {
              requestId,
              venueId,
              paymentId: variante.tipo === 'COLISION_DE_REFERENCIA' ? variante.paymentId : null,
              reason: contradiccion,
            },
          )
      ...
      const { row, previousFailureCode, otros, solicitudesSinDesenlace, ligados, afirmacion } = retenida
      ...
      // Ronda 3: las dos señales nuevas. Mismo cuerpo (monto, cobros posteriores, recobros en vuelo y qué hacer) y su propia
      // primera línea: la del brief para la colisión, y la de la afirmación de la terminal.
      if (variante.tipo === 'COLISION_DE_REFERENCIA' || variante.tipo === 'AFIRMACION_DE_LA_TERMINAL') {
        logger.error(`🚨 [TerminalPayment] ${marca.grito}`, contexto)
        void sendOpsAlert({
          subject: `${marca.asunto} — ${row.terminalId}`,
          lines: [
            variante.tipo === 'COLISION_DE_REFERENCIA'
              ? `Un registro con la referencia de otro cobro (evidencia ${variante.paymentId}) llegó sobre la solicitud ${requestId} de ${monto} (orden ${row.orderId ?? 'sin orden'}) después de que ${liberacion}, y quedó como evidencia pendiente (origen ${variante.origen}).`
              : `La terminal AFIRMÓ haber cobrado la solicitud ${requestId} de ${monto} (orden ${row.orderId ?? 'sin orden'}) después de que ${liberacion}, y no hay Payment que lo acredite (${JSON.stringify(afirmacion ?? {})}).`,
            lineaDeCobros,
            ...lineasDeRecobro,
            'La solicitud volvió a TIMED_OUT/PAYMENT_UNBOUND_AWAITING_REVIEW: la venta queda bloqueada y la terminal se suelta a los 20 min. Confirmar en el portal del procesador si hubo cargo y conciliar a mano.',
          ],
        })
        return 'HELD'
      }
      ...
      logger.warn(
        porAprobacion
          ...
          : variante.tipo === 'PAGO_SIN_LIGAR'
            ...
            : `⏱️ [TerminalPayment] re-hold of a released request (${variante.tipo}) deferred — the next caller retries it`,
  ...
  /**
   * Qué se hace con UNA fila ya liberada sobre la que hay dinero: primero el cierre COMÚN (si el Payment es atribuible la
   * fila se CONCILIA a COMPLETED, que es lo correcto y lo que ya hacía el barrido de 30 min), y sólo si no se pudo LIGAR —o
   * si no hay Payment atribuible pero SÍ uno ligado (la guarda G1 tras liberar)— se RE-RETIENE para revisión.
   *
   * Ronda 3 (P1-A): vive en UN sitio porque ahora lo comparten el barrido de 30 min y la RED DURABLE. Si cada uno tuviera su
   * copia, el de la red podría acabar reteniendo para revisión un cobro que el otro habría conciliado — dos veredictos
   * distintos sobre el mismo dinero según quién llegara primero.
   */
  private async conciliarORetenerLiberada(
    row: FilaDeCobroRemoto,
    origen: OrigenDeRetencionPorPago,
  ): Promise<'RECONCILED' | 'HELD' | 'NADA'> {
    const retener = async (paymentId: string) =>
      (await this.retenerSolicitudLiberadaPorPagoSinLigar({ requestId: row.requestId, venueId: row.venueId, paymentId, origen })) === 'HELD'
        ? ('HELD' as const)
        : ('NADA' as const)
    const payment = await this.findReconcilablePayment(row)
    if (payment) {
      ...
      if (cierre.reason === 'ALREADY_BOUND') return 'NADA'
      return retener(payment.id)
    }
    ...
    return ligado ? retener(ligado.id) : 'NADA'
  }
  ...
  /**
   * Codex r1 (P1-B): recorre TODAS las filas de `where` por keyset (`updatedAt`, `id`), en lotes de 200 y con tope de 25 lotes
   * por pasada (5 000 filas; si se alcanza se avisa y el resto espera a la siguiente pasada). Sin cursor, el barrido releía las
   * primeras 200 en cada pasada y las de atrás —cuyo Payment sólo está etiquetado en `processorData`— podían cumplir los 30
   * minutos sin revisarse nunca.
   */
  private async paginarLiberadas(
    where: Prisma.TerminalPaymentRequestWhereInput,
    contexto: string,
    fn: (row: FilaDeCobroRemoto) => Promise<void>,
  ): Promise<void> {
    type Cursor = { updatedAt: Date; id: string } | null
    let cursor: Cursor = null
    for (let lote = 0; lote < LOTES_MAXIMOS_DE_LIBERADAS; lote++) {
      const desde: Cursor = cursor
      const rows: FilaDeCobroRemoto[] = await retry(
        () =>
          prisma.terminalPaymentRequest.findMany({
            where: desde
              ? { ...where, OR: [{ updatedAt: { gt: desde.updatedAt } }, { updatedAt: desde.updatedAt, id: { gt: desde.id } }] }
              : where,
            orderBy: [{ updatedAt: 'asc' }, { id: 'asc' }],
            take: TAMANO_DEL_LOTE_LIBERADAS,
          }),
        { retries: 3, shouldRetry: shouldRetryDbConnectionError, context: contexto },
      )
      for (const row of rows) await fn(row)
      if (rows.length < TAMANO_DEL_LOTE_LIBERADAS) return
      const last = rows[rows.length - 1]
      cursor = { updatedAt: last.updatedAt, id: last.id }
      if (lote === LOTES_MAXIMOS_DE_LIBERADAS - 1)
        logger.warn('⚠️ [Terminal-payment watchdog] released-rows sweep hit the batch cap — the rest waits for the next pass', {
          contexto,
          lotes: LOTES_MAXIMOS_DE_LIBERADAS,
          filas: LOTES_MAXIMOS_DE_LIBERADAS * TAMANO_DEL_LOTE_LIBERADAS,
        })
   ...
   */
  async reconcileUnknownRequests(now: Date = new Date()): Promise<{
    completed: number
    marked: number
    released: number
    reset: number
    ...
    await this.paginarLiberadas(
      ...
      'terminal-payment-watchdog:findReleased',
      async row => {
        const payment = await this.findReconcilablePayment(row)
        if (!payment) return
    ...
    for (const failureCode of CODIGOS_DE_LIBERACION_REVERSIBLE) {
      await this.paginarLiberadas(
        ...
        async row => {
          if ((await this.conciliarORetenerLiberada(row, 'BARRIDO_LIBERADAS')) === 'RECONCILED') lateReconciled += 1
        },
      )
    }
  ...
  async handleAttemptOpenedFromSocket(
    ...
    const { OPCIONES_DE_TRANSACCION_DEL_INTENTO, candadoDeIntento, candadoDeSolicitud } = await import('./tpv/candadoDeIntento')
    try {
      await prisma.$transaction(async tx => {
        // Codex r1 (P1-C): el candado de la SOLICITUD va antes que el del intento (orden fijo solicitud → intentos, el mismo de
        // la ventana): un vínculo no se publica mientras la ventana decide sobre su solicitud, y la ventana que llegue después
        // lo ve al enumerar los vínculos bajo ese mismo candado.
        await candadoDeSolicitud(tx, requestId)
        // Codex R6-2: la EXCLUSIÓN por intento va ANTES del INSERT y de consultar eventos. Un escritor débil que
  ...
  async handleProbeResultFromSocket(
    ...
    if (disposition === 'RESOLVED' || disposition === 'RECEIVED_CANCELLED') {
      ...
      // tarjeta se cobró: se audita UNA vez lo que la terminal guardó (evidencia «clase B») y no se vuelve a preguntar en la
      // ventana de espera — PERO el resultado sigue a `closeRow` (Codex r1, P2): un negativo DURABLE sin evidencia se degrada
      // a `timeout` + sobre ⇒ TIMED_OUT ⇒ la VENTANA DE CONFIRMACIÓN decide (webhook, Payment, cajero, o liberar a los 30 s).
      // Antes se retornaba aquí y una fila UNKNOWN cuyo resultado original se perdió nunca entraba a la ventana. La razón
      // histórica («closeRow re-marcaba UNKNOWN en cada barrido», medido en la PAX 2841548417 el 10-sep) ya no aplica:
      // `closeRow` escribe una vez (CAS en vuelo / tardío) y el backoff evita la segunda sonda. Un `success` entra igual:
      ...
      if (!acreditado) {
        ...
        logger.warn('🔎 [TerminalPayment] Probe answer carries no accredited evidence: handed to the confirmation window', {
          requestId,
          terminalId: row.terminalId,
          status: row.status,
