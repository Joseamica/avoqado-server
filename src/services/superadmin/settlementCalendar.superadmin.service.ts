...
/**
 * Página del recorrido de pagos (query-guard 2026-09-18).
 *
 * Este calendario es el ÚNICO camino del motor que no filtra por negocio: recorre TODOS los
 * negocios de la plataforma, con `venue` y `merchantAccount` (proveedor + agregador)
 * colgando de cada pago. Medido en producción del 15 al 18-sep: 2,207 a 2,462 filas por
 * llamada con sólo dos negocios activos — y crece con cada cliente nuevo, no con el tiempo.
 * El server corre en UNA instancia con UNA vCPU (`.claude/rules/una-sola-instancia.md`).
 *
 * ⚠️ Se pagina y NO se agrega en SQL porque la fecha de caída la calcula el motor en JS, por
 * pago y en la zona horaria de SU negocio: no cabe en un GROUP BY.
 */
const SETTLEMENT_CALENDAR_PAGE_SIZE = 500
...
export async function getCrossVenueSettlementCalendar(fromKey: string, toKey: string): Promise<CrossVenueSettlementCalendar> {
  ...
  while (true) {
    const pagina = await prisma.payment.findMany({
      ...
      take: SETTLEMENT_CALENDAR_PAGE_SIZE,
    ...
    if (pagina.length < SETTLEMENT_CALENDAR_PAGE_SIZE) break
