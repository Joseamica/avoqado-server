...
**Goal:** Que un webhook de Uber Eats llegue a `/api/v1/webhooks/delivery/uber`, se verifique su firma HMAC, se deduplique y quede
persistido como `DeliveryOrderEvent` — probado end-to-end firmándonos los eventos nosotros (sin depender de Uber).

**Architecture:** Camino propio de Uber (Deliverect CONGELADO — no tocar `providers/deliverect/**` ni el core que consume). Módulos puros y
testeables; el HTTP de más bajo nivel pasa por el candado `uber.storeAllowlist` ya construido (6/6 verde).
...
⚠️ Esta task construye SOLO el caché. El adaptador HTTP real (`POST /oauth/v2/token` con `client_id`/`client_secret`, el par inseparable
sandbox-login↔test-api vs auth↔api, y su env tipada en `src/config/env.ts` — hoy NO existe ninguna var `UBER_*` ahí) es task del Plan 2:
sin tienda de prueba no hay contra qué validarlo. El caché asume UNAS credenciales por proceso (no usar con múltiples client_id) y es por
PROCESO: válido mientras producción sea una sola instancia (`render.yaml`, regla `una-sola-instancia.md`); antes de habilitar PM2 cluster
(`ecosystem.config.js` tiene `instances:'max'` dormido) hay que promoverlo a caché compartido.
...
- Produces: `getUberAppToken(deps: UberTokenDeps): Promise<string>` y `_resetUberTokenCacheForTests()`.
  `UberTokenDeps = { fetchToken: () => Promise<{ access_token: string; expires_in: number }>, now?: () => number }`. Task 2 NO lo consume
  (webhook no llama a Uber); lo consumirá el Plan 2 (fetchOrder).
...
describe('uber.token', () => {
  ...
  it('pide UNA vez y reusa el token mientras no expira', async () => {
    ...
    const deps = {
      fetchToken: async () => {
        calls++
        return { access_token: `tok-${calls}`, expires_in: 2592000 }
      },
      now: () => 1_000_000,
    }
  ...
  it('respuesta inválida del proveedor ⇒ rechaza y NO la cachea', async () => {
    ...
    const deps = {
      fetchToken: async () => {
        calls++
        return { access_token: '', expires_in: 2592000 }
      },
    }
  ...
  it('si la petición falla, la siguiente REINTENTA (no cachea el error)', async () => {
    ...
    const deps = {
      fetchToken: async () => {
        ...
        return { access_token: 'tok-ok', expires_in: 2592000 }
      },
    }
...
Run: `npx jest --selectProjects unit --testPathPattern "uberToken"` Expected: FAIL — "Cannot find module .../uber.token"
...
Run: `npx jest --selectProjects unit --testPathPattern "uberToken"` Expected: PASS — 5 passed
...
- Consumes: nada
- Produces: `verifyUberSignature(rawBody: Buffer, headerValue: string | undefined, signingKey: string): boolean`. La consume Task 3
  (controller).
...
Run: `npx jest --selectProjects unit --testPathPattern "uberSignature"` Expected: FAIL — módulo inexistente
...
Run: `npx jest --selectProjects unit --testPathPattern "uberSignature"` Expected: PASS — 4 passed
...
Pendiente de detallar con el mismo formato ANTES de ejecutarse (auditoría Codex 2026-08-17: tal como estaba esbozada no era ejecutable).
Requisitos que el detalle DEBE incluir:

- Migración aditiva a `DeliveryOrderEvent`: `externalOrderId`, `dedupKey @unique`, **y también `claimToken` + `lockedUntil`** (contrato
  recuperable de la spec §7 — el esbozo los omitía)
- `UBER_WEBHOOK_SIGNING_KEY` en la env TIPADA (`src/config/env.ts` + `.env.example`): está en el `.env` local pero NO en el schema Zod — sin
  esto el server ni arranca con la var
- `npm run schema:map` + `docs/SCHEMA_MAP.md` en el MISMO cambio (critical-warnings.md)
- Controller: `res.status(200).end()` — NUNCA `sendStatus(200)`, que manda body "OK" y Uber espera body VACÍO
- 🔴 La prueba durable vive en el proyecto **integration** con PostgreSQL real (TEST_DATABASE_URL): `unit` y `api-tests` mockean Prisma
  globalmente (`setup.ts:361`), así que ahí el unique de dedup NO se prueba de verdad. ngrok queda como smoke manual, no como prueba
  automatizada
- Tests mínimos: tienda desconocida ⇒ fila con tenant/link nulos + 200 vacío · dos POST concurrentes con el mismo `event_id` ⇒ UNA fila
- 🔴 NO reutilizar `deliveryWebhookEvent.service.ts` (su contrato exige `channelLinkId` conocido y deduplica con semántica Deliverect —
  Deliverect congelado)
- `prisma migrate dev` con el founder presente

## Self-Review

- Cobertura: paso 2 **PARCIAL** (Task 1 = solo caché; adaptador HTTP real → Plan 2) · paso 3 firma (Task 2) ✓ · paso 3 persistencia (Task 3:
  requisitos fijados, detalle antes de ejecutar)
