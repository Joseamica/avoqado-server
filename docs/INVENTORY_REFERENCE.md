...
- `InterVenueTransferVarianceResolution` closes shortages explicitly. `NOT_DISPATCHED` has no value impact; in-transit loss/damage uses the
  remaining FIFO value.
...
`TRANSFER_OUT` and `TRANSFER_IN` movements; manual adjustment endpoints reject both values. Entitlement is checked for source and
destination, and consolidated inventory includes only venues the actor can access that also have `INVENTORY_TRACKING`.
