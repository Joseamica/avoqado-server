...
| Mover una terminal de tienda                   | `mover-terminal-bait.ts` + borrado de fábrica | 🟢 **YA tiene autoservicio**: el asistente «Migrar a otra sucursal» del dashboard de org (`OrgTerminalDrawer`) — Isaac lo usa solo. ⚠️ Los movimientos a mano de abril dejaron 6 comandos `MANUAL_DB` sin `expiresAt` que lo bloquearon hasta el 2026-09-01 (Asana 1218069201250971): **si insertas en `TpvCommandQueue` a mano, pon `expiresAt`** |
