...
2. **Rate limiters → `rate-limit-redis`** (ya está en `package.json`, sin usar). Los 32 sitios **más el store propio de `mcp-rate-limit.middleware.ts`** (no es un `rateLimit({...})`; se migra cambiando su clase `RateLimitStore`).
