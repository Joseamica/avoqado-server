...
const VENUE = 'venue-1'
const ACTOR = 'owner-1'
const abre = { id: 'tpl-abre', venueId: VENUE, name: 'Abre', abbreviation: 'AB', color: '#7ADD2C', startTime: '08:00', endTime: '16:00', active: true }
...
describe('plantillas', () => {
  it('crea una plantilla con nombre, abreviatura, color y horario', async () => {
    const t = await createTemplate(VENUE, { name: 'Cierre', abbreviation: 'CI', color: '#123456', startTime: '11:00', endTime: '19:00' }, ACTOR)
    expect((prismaMock as any).workShiftTemplate.create).toHaveBeenCalledWith({ data: expect.objectContaining({ venueId: VENUE, name: 'Cierre', startTime: '11:00', endTime: '19:00' }) })
    expect(t.name).toBe('Cierre')
  })
  it('🔴 empezar y terminar a la misma hora NO es un turno', async () => {
    await expect(createTemplate(VENUE, { name: 'X', abbreviation: 'X', startTime: '09:00', endTime: '09:00' }, ACTOR)).rejects.toMatchObject({ statusCode: 400 })
  })
  it('un turno nocturno (termina antes de empezar) SÍ es válido', async () => {
    await expect(createTemplate(VENUE, { name: 'Noche', abbreviation: 'NO', startTime: '22:00', endTime: '06:00' }, ACTOR)).resolves.toBeTruthy()
  })
...
describe('asignaciones', () => {
  ...
  it('templateId null BORRA la asignación de esa celda', async () => {
    ...
    expect((prismaMock as any).workShiftAssignment.deleteMany).toHaveBeenCalledWith({ where: { venueId: VENUE, staffVenueId: 'sv-1', date: '2026-08-25' } })
  ...
  it('🔴 una fecha fuera de la ventana pedida se rechaza (no se escribe nada de otra semana por accidente)', async () => {
    await expect(replaceAssignments(VENUE, { from: '2026-08-24', to: '2026-08-30', items: [{ staffVenueId: 'sv-1', date: '2026-09-02', templateId: 'tpl-abre' }] }, ACTOR)).rejects.toMatchObject({ statusCode: 400 })
  })
  it('🔴 una plantilla de OTRO venue (o inactiva) se rechaza', async () => {
    ;(prismaMock as any).workShiftTemplate.findMany.mockResolvedValue([])
    await expect(replaceAssignments(VENUE, { from: '2026-08-24', to: '2026-08-30', items: [{ staffVenueId: 'sv-1', date: '2026-08-25', templateId: 'tpl-ajeno' }] }, ACTOR)).rejects.toMatchObject({ statusCode: 400 })
  })
  it('🔴 un empleado que no es de este venue se rechaza', async () => {
    await expect(replaceAssignments(VENUE, { from: '2026-08-24', to: '2026-08-30', items: [{ staffVenueId: 'sv-otro', date: '2026-08-25', templateId: 'tpl-abre' }] }, ACTOR)).rejects.toMatchObject({ statusCode: 400 })
  })
