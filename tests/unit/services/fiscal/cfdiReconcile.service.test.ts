...
import { ProviderInvoiceSummary, StampedInvoice } from '../../../../src/services/fiscal/providers/fiscal-provider.interface'
...
describe('reconcileStuckCfdi', () => {
  ...
  describe('COMPLETED (orphaned stamp recovered)', () => {
    it('search by reference finds a valid match → downloads artifacts and marks STAMPED', async () => {
      ...
      // searched, not getInvoice (no facturapiId on the row)
    ...
    it('facturapiId present → uses getInvoice(id) and completes when valid', async () => {
      ...
      const deps = makeDeps(provider)
      const row: StuckCfdi = { ...STUCK_INDIVIDUAL, facturapiId: 'fp1' }
  ...
  describe('RESET (no stamp at PAC, safe to retry)', () => {
    ...
    it('getInvoice throws a not-found error → reset', async () => {
      ...
      const deps = makeDeps(provider)
      const row: StuckCfdi = { ...STUCK_INDIVIDUAL, facturapiId: 'ghost' }
  ...
  describe('regression: outcomes are mutually exclusive', () => {
    it('COMPLETED never also resets, RESET never also completes', async () => {
      // COMPLETED path
      const p1 = makeProvider({ searchInvoices: jest.fn().mockResolvedValue({ invoices: [MATCH_INDIVIDUAL], truncated: false }) })
