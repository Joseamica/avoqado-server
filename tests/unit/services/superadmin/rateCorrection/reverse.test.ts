 ...
 * candidatos —tipo, venue y puntero— por `$queryRaw`). El doble contesta ESA consulta con las filas de `p1`/`p2` (REGULAR, del venue
...
const fotografia = (sql: unknown, ids: string[]) =>
  Array.isArray(sql) && sql.join('?').includes('AS "original"')
    ...
    : []
const tx = {
  // Codex R12-4: bajo el mutex se revalida `cobrosDelProtocolo` — sin protocolo ⇒ [].
  $queryRaw: jest.fn(async (sql: unknown) => fotografia(sql, ['p1', 'p2'])),
  $executeRaw: jest.fn(),
