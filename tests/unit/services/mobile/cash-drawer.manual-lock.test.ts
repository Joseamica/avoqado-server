...
describe('movimiento manual bajo candado', () => {
  it('🔴 si la caja se cerró entre leerla e insertar, el retiro se rechaza (404) y NO se escribe', async () => {
    ...
    await expect(payOut({ venueId: VENUE, staffId: 'st', staffName: 'Ana', amount: 100 })).rejects.toMatchObject({ statusCode: 404 })
  ...
  it('con la caja abierta el candado se toma con status=OPEN y luego se inserta', async () => {
    ...
    await payOut({ venueId: VENUE, staffId: 'st', staffName: 'Ana', amount: 100 })
...
describe('cierre encolado con sessionId', () => {
  it('🔴 si la caja abierta ahora es OTRA, no se cierra con el conteo de la vieja: 404', async () => {
    ;(prismaMock as any).cashDrawerSession = {
      update: jest.fn().mockResolvedValue({}),
      findFirst: jest.fn().mockResolvedValue({ id: 's-2' }),
      updateMany: jest.fn(),
    }
    await expect(
      closeSession({ venueId: VENUE, staffId: 'st', staffName: 'Ana', actualAmount: 300, sessionId: 's-1' }),
