...
const mockStockCountFindMany = jest.fn()
...
jest.mock('@/utils/prismaClient', () => ({
  ...
  default: {
    stockCount: {
      findFirst: (...a: unknown[]) => mockStockCountFindFirst(...(a as [])),
      findMany: (...a: unknown[]) => mockStockCountFindMany(...(a as [])),
    },
...
const callList = (args: Record<string, unknown>) => handlers.get('stock_counts')!(args, {})
...
describe('cancel_stock_count', () => {
  ...
  it('sin confirm → vista previa con el conteo y NO cancela', async () => {
    ...
    expect(out.expectedRevision).toBe(4)
  ...
  it('confirm:true → cancela con el staff conectado y audita', async () => {
    ...
    const out = parse(await call({ venueId: 'v1', countId: 'c1', expectedRevision: 4, confirm: true }))
  ...
  it('confirm:true sin repetir expectedRevision no ejecuta la cancelación', async () => {
    mockStockCountFindFirst.mockResolvedValueOnce({
      id: 'c1',
      revision: 4,
      status: 'IN_PROGRESS',
...
describe('stock_counts revision contract', () => {
  it('exposes the revision used by a later guarded mutation', async () => {
