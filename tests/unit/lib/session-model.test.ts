...
describe('modelo Session', () => {
  it('expone los campos que el carril de sesiones necesita', () => {
    ...
    expect(campos).toEqual(expect.arrayContaining([
      'id', 'staffId', 'venueId', 'deviceId', 'authMethod',
      'parentSessionId', 'createdAt', 'lastSeenAt', 'revokedAt', 'revokedReason',
    ]))
  })
  ...
  it('RefreshGrant permite rotación, familia y retransmisión', () => {
    ...
    expect(campos).toEqual(expect.arrayContaining([
      'id', 'sessionId', 'familyId', 'tokenHash',
      'successorEnc', 'successorEncExpiresAt', 'consumedAt', 'rotatedToId', 'expiresAt', 'revokedAt',
    ]))
  })
