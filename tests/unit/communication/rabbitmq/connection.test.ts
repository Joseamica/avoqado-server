...
describe('RabbitMQ Connection', () => {
  ...
  beforeEach(() => {
    ...
    mockConnectionObj = {
      // This is the Connection instance
      on: jest.fn(),
      close: jest.fn().mockResolvedValue(undefined),
    }
    mockChannelModel = {
      // This is what amqplib.connect resolves to
  ...
  describe('Connection Failure and Retry Logic', () => {
    ...
    it('should successfully connect on a retry attempt', async () => {
      const connectError = new Error('Initial connection failed')
      mockAmqplibConnect
        .mockRejectedValueOnce(connectError) // First attempt fails
    ...
    it('should handle multiple sequential failures before a successful retry', async () => {
      ...
      mockAmqplibConnect.mockRejectedValueOnce(error1).mockRejectedValueOnce(error2).mockResolvedValueOnce(mockChannelModel) // Third attempt succeeds
